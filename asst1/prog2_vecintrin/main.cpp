...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float upper_f = _cs149_vset_float(9.999999f);
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    _cs149_vload_float(vals, values+i, maskAll);               // x = values[i];
    _cs149_vload_int(exps, exponents+i, maskAll);              // y = exponents[i];

    _cs149_veq_int(maskIsZero, exps, zero, maskAll);           // if (y == 0) {

    _cs149_vmove_float(result, one_f, maskIsZero);             //    output[i] = 1.0f;

    maskIsNotZero = _cs149_mask_not(maskIsZero);               // } else {

    _cs149_vmove_float(result, vals, maskIsNotZero);           //    result = x;
    _cs149_vsub_int(exps, exps, one_i, maskIsNotZero);         //    exps = exps - 1;

    // Loop Logic
    _cs149_veq_int(maskIsZero, exps, zero, maskIsNotZero);
    maskIsNotZero = _cs149_mask_not(maskIsZero);
    int res_not_zero = 0;
    if(i + VECTOR_WIDTH >= N) {
      res_not_zero = VECTOR_WIDTH - (N - i);
    }
    while(_cs149_cntbits(maskIsNotZero) != res_not_zero) {     //    while (count > 0) {
      _cs149_vmult_float(result, result, vals, maskIsNotZero); //    result *= x;
      _cs149_vsub_int(exps, exps, one_i, maskIsNotZero);       //    exps = exps - 1;
      _cs149_veq_int(maskIsZero, exps, zero, maskIsNotZero);
      maskIsNotZero = _cs149_mask_not(maskIsZero);
    }                                                          //    }

    _cs149_vgt_float(maskAll, result, upper_f, maskAll);       //    if (result > 9.999999f) {
    _cs149_vmove_float(result, upper_f, maskAll);              //      result = 9.999999f;
...
float arraySumVector(float* values, int N) {
  ...
  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    ...
    for(int j = 0; j < VECTOR_WIDTH; j+= 2) {
      ...
      _cs149_interleave_float(vals, vals);
    }
    _cs149_vadd_float(sum, sum, vals, maskAll);
  }
