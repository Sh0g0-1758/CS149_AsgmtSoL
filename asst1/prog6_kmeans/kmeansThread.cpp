...
//   int chunk = args->M / NUM_THREADS;
//   for(int i = 1; i < NUM_THREADS; i++) {
//     workers[i] = std::thread(worker_function, args, i * chunk, (i + 1) * chunk, minDist);
//   }
...
void worker_function(WorkerArgs *const args, int start, int end, double *minDist, int k) {
...
void computeAssignments(WorkerArgs *const args) {
  ...
  for (int k = args->start; k < args->end; k++) {
    ...
    for(int i = 0; i < NUM_THREADS; i++) {
      workers[i] = std::thread(worker_function, args, i * chunk, (i + 1) * chunk, minDist, k);
    }
    for(int i = 0; i < NUM_THREADS; i++) {
      workers[i].join();
    }
...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  int iter = 0;
  double compute_asssignments_time = 0;
  double compute_centroids_time = 0;
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    compute_asssignments_time += endTime - startTime;
    startTime = CycleTimer::currentSeconds();
    computeCentroids(&args);
    endTime = CycleTimer::currentSeconds();
    compute_centroids_time += endTime - startTime;
    startTime = CycleTimer::currentSeconds();
    computeCost(&args);
    endTime = CycleTimer::currentSeconds();
    compute_cost_time += endTime - startTime;
    iter++;
  }

  printf("[Compute Assignments Time]: %.3f ms\n", compute_asssignments_time * 1000);
  printf("[Compute Centroids Time]: %.3f ms\n", compute_centroids_time * 1000);
  printf("[Compute Cost Time]: %.3f ms\n", compute_cost_time * 1000);
